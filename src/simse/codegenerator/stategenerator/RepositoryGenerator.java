  ...
  private void generateMetaObjectTypeRepository(String typeName) {
    ...
    if(typeName.equals("Employee")) {
        ...
        MethodSpec createInstanceOneState = MethodSpec.methodBuilder("createInstance")
        		.addModifiers(Modifier.PUBLIC, Modifier.STATIC)
        		.addParameter(state, "state")
        		.addStatement("instances.put(state, new $T())", typeStateRepo)
        		.build();
        ...
        MethodSpec createInstanceOldState = MethodSpec.methodBuilder("createInstance")
        		...
        		.addStatement("$T oldInstance = instances.get(oldState)", typeStateRepo)
        		.addStatement("instances.put(newState, ($T)oldInstance.clone())", typeStateRepo)
        		.build();
    	...
    	repStateRepoBuilder.addMethod(getInstance)
    					...
    					.addField(FieldSpec.builder(
    							ParameterizedTypeName.get(hashMap, state, typeStateRepo), "instances", Modifier.PRIVATE, Modifier.STATIC)
