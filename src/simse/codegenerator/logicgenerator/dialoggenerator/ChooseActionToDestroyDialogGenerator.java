...
public class ChooseActionToDestroyDialogGenerator implements CodeGeneratorConstants {
	...
	public ChooseActionToDestroyDialogGenerator(DefinedActionTypes actTypes, File directory) {
	...
	public void generate() {
		...
		MethodSpec destroyConstructor = MethodSpec.constructorBuilder().addModifiers(Modifier.PUBLIC)
				.addParameter(stageClass, "parent").addParameter(actionsType, "acts").addParameter(stateClass, "s")
				.addParameter(employeeClass, "e").addParameter(ruleExecClass, "r").addParameter(String.class, "mText")
				.addStatement("$N = acts", "actions").addStatement("$N = s", "state").addStatement("$N = r", "ruleExec")
				.addStatement("$N = parent", "gui").addStatement("$N = e", "emp").addStatement("$N = mText", "menuText")
		...
		TypeSpec destroyDialog = TypeSpec.classBuilder("ChooseActionToDestroyDialog").addModifiers(Modifier.PUBLIC)
				...
				.addField(buttonClass, "cancelButton", Modifier.PRIVATE).addMethod(destroyConstructor).addMethod(handle)
				.build();

		JavaFile javaFile = JavaFile.builder("", destroyDialog).build();
		...
		try {
			...
			JOptionPane.showMessageDialog(null, ("Error writing file " + catddFile.getPath() + ": " + e.toString()),
	...
	private String generateNames(Vector<ActionType> userDestActs) {
		...
		for (int j = 0; j < userDestActs.size(); j++) {
			...
			names += "if(tempAct instanceof " + CodeGeneratorUtils.getUpperCaseLeading(act.getName()) + "Action)\n{\n";
			names += "actionName = \"" + CodeGeneratorUtils.getUpperCaseLeading(act.getName()) + "\";\n}\n";
		}
	...
	private String generateActionConstructor(Vector<ActionType> userDestActs) {
		...
		for (int j = 0; j < userDestActs.size(); j++) {
			...
			actions += "if(tempAct instanceof " + CodeGeneratorUtils.getUpperCaseLeading(act.getName())
					+ "Action)\n{\n";
			actions += "for(int i=0; i<actions.size(); i++)\n{\n";
			actions += CodeGeneratorUtils.getUpperCaseLeading(act.getName()) + "Action act = ("
					+ CodeGeneratorUtils.getUpperCaseLeading(act.getName()) + "Action)actions.elementAt(i);\n";
			...
			for (int k = 0; k < parts.size(); k++) {
				...
				actions += "Vector<" + SimSEObjectTypeTypes.getText(tempPart.getSimSEObjectTypeType()) + "> all"
						+ tempPart.getName() + "s = act.getAll" + tempPart.getName() + "s();\n";
				actions += "for(int j=0; j<all" + tempPart.getName() + "s.size(); j++)\n{\n";
				actions += "if(j > 0)\n{\n";
				actions += "label.append(\", \");\n}\n";
				actions += SimSEObjectTypeTypes.getText(tempPart.getSimSEObjectTypeType()) + " a = all"
						+ tempPart.getName() + "s.elementAt(j);\n";
				...
				for (int m = 0; m < ssObjTypes.size(); m++) {
					...
					actions += "if(a instanceof " + CodeGeneratorUtils.getUpperCaseLeading(tempType.getName())
							+ ")\n{\n";
					actions += "label.append(\"" + CodeGeneratorUtils.getUpperCaseLeading(tempType.getName())
	...
	private String generateActionHandle(Vector<ActionType> userDestActs) {
		...
		for (int i = 0; i < userDestActs.size(); i++) {
			...
			actions += "if(tempAct instanceof " + CodeGeneratorUtils.getUpperCaseLeading(tempAct.getName())
					+ "Action)\n{\n";
			actions += CodeGeneratorUtils.getUpperCaseLeading(tempAct.getName()) + "Action "
			...
			for (int j = 0; j < parts.size(); j++) {
				...
				if (part.getSimSEObjectTypeType() == SimSEObjectTypeTypes.EMPLOYEE) {
					actions += tempAct.getName().toLowerCase() + "Act.remove" + part.getName() + "(emp);\n";
				}
			}
				...
				if ((tempDest instanceof UserActionTypeDestroyer) && (tempDest.getDestroyerText() != null)
					...
					for (int k = 0; k < destRules.size(); k++) {
						...
						if (dRule.getExecuteOnJoins() == true) {
							actions += "ruleExec.update(gui, RuleExecutor.UPDATE_ONE, \"" + dRule.getName() + "\", "
									+ tempAct.getName().toLowerCase() + "Act);\n";
						}
					}
					actions += "}\n";
			...
			for (int j = 0; j < parts.size(); j++) {
				...
				ActionTypeParticipant part = parts.elementAt(j);
				actions += "if(" + tempAct.getName().toLowerCase() + "Act.getAll" + part.getName() + "s().size() < ";
				...
				actions += ")\n{\n";
				actions += "Vector<SSObject> c = " + tempAct.getName().toLowerCase() + "Act.getAllParticipants();\n";
				...
				actions += "state.getActionStateRepository().get"
						+ CodeGeneratorUtils.getUpperCaseLeading(tempAct.getName()) + "ActionStateRepository().remove("
						+ tempAct.getName().toLowerCase() + "Act);\n";
				...
				if (tempAct.hasGameEndingDestroyer()) {
					...
					for (int k = 0; k < allDests.size(); k++) {
						...
						if ((tempDest instanceof UserActionTypeDestroyer) && (tempDest.isGameEndingDestroyer())) {
							...
							actions += CodeGeneratorUtils.getUpperCaseLeading(tempAct.getName()) + "Action t111 = ("
							...
							if ((scoringAttConst != null) && (scoringPartConst != null) && (scoringPartDest != null)) {
								actions += "if(t111.getAll" + scoringPartDest.getParticipant().getName()
										+ "s().size() > 0)\n{\n";
