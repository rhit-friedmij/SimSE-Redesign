...
public class ParticipantSelectionDialogsDriverGenerator implements
  ...
  public void generate() {
	  ...
	  MethodSpec participantConstructor = MethodSpec.constructorBuilder()
			  ...
			  .beginControlFlow("if (tempObj == tempObj2) ")
			  .addStatement("participants.remove(tempObj)")
			  .addStatement("break")
			  .endControlFlow()
			  .endControlFlow()
			  .endControlFlow()
			  .beginControlFlow("if ((participants.size() == 0) || (participants."
			  		+ "elementAt(0) instanceof $T)) ", employeeClass)
			  ...
			  .addStatement("participantsContainsSelEmp = true")
			  .addStatement("break")
			  .endControlFlow()
			  .endControlFlow()
			  .beginControlFlow("if (($N != null) && (participantsContainsSelEmp)) ", "selectedEmp")
			  .addStatement("participants.remove($N)", "selectedEmp")
			  .addStatement("$T psd = new $T(parent, participantName, new $T(participants), $N,"
			  		+ " $N, $N)", empPartDialog, empPartDialog, ssObjectVector, "action",
			  		"state", "selectedEmp")
			  ...
			  .endControlFlow()
			  .nextControlFlow(" else ")
			  .addStatement("$T psd = new $T(parent, participantName, new $T(participants),"
			  		+ " $N, $N)", nonEmpPartDialog, nonEmpPartDialog, ssObjectVector, "action", "state")
			  .beginControlFlow("if (psd.actionCancelled()) ")
			  .addStatement("actionValid = false")
			  .addStatement("break")
			  .endControlFlow()
